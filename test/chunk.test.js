...
describe("Chunk", () => {
    it("returns the example output when called with the example inputs", () => {
        expect(chunk(['a', 'b', 'c', 'd'], 2)).to.be.an('array').that.deep.includes(['a', 'b']).and.deep.includes(['c', 'd']);
        expect(chunk(['a', 'b', 'c', 'd'], 3)).to.be.an('array').that.deep.includes(['a', 'b', 'c']).and.deep.includes(['d']);
    })
    ...
    it("can handle nested arrays", () => {
        expect(chunk([['a', 'b'], 'c', 'd'], 2)).to.be.an('array').that.deep.includes([['a', 'b'], 'c']).and.deep.includes(['d']);
        expect(chunk(['a', ['b', 'c'], 'd'], 3)).to.be.an('array').that.deep.includes(['a']).and.deep.includes(['b', 'c']).and.deep.includes(['d']);
    })
